...
conf = SparkConf().setAppName("dev-PySparkStreaming") \
...
def grid_block(lng_x, lat_y):
    if not (120.04 <= lng_x < 122.0 and 21.9 <= lat_y < 25.3):
        return (-1, -1)
    cor_x = math.floor(round((lng_x - 120.04) / 0.02, 4))
    cor_y = math.floor(round((lat_y - 21.9) / 0.02, 4))
    grid = 98 * cor_y + cor_x

    dx = lng_x - (120.04 + 0.02 * cor_x + 0.01)
    dy = lat_y - (21.9 + 0.02 * cor_y + 0.01)
    if (dx >= 0 and dy >= 0):
        block = 1
    elif (dx < 0 and dy >= 0):
        block = 2
...
def process(time, rowRdd):
    ...
    try:
        ...
        domain = "13.73.26.38"
        url = "http://{}:3853/passengersradar/realtimeData/".format(domain)
